...
export const tfToNumber = (inp: Timeframe) => {
	const s = 1000;
	const m = 60 * s;
	const h = 60 * m;
...
export const isCandleLengthValid = (diff: number, timeframe: Timeframe) => {
	const frame = tfToNumber(timeframe);
	const count = diff / frame;
	return count < 25_000;
};

export const setTimeFrame = (candles: Candle[], tf: Timeframe) => {
...
export const fillBlanks = (candles: Candle[], frame: number) => {
